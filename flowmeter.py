...
class Flowmeter:
    ...
    def run(self):
        ...
        if self.offline:
            ...
            results = sniff(prn=self.process_packet,count=10)
            print('Stopping...')
...
ff = Flowmeter(offline=sys.argv[1],outfunc = None,outfile=sys.argv[2]) #None - debug ; Nope - production
ff.run()
...
while(True):
    ...
    ff = Flowmeter(outfunc = None,outfile=filebase.format(i%20))
    ff.run()
    i = i+1
'''